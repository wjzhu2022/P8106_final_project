...
#data partition
set.seed(2023)
train_index=createDataPartition(y = dat$recovery_t,
                                p = 0.8,
                                list = FALSE)
#training data
train_dat=dat[train_index,]
x1 = model.matrix(recovery_t~., data=dat)[train_index,-1]
y1=dat$recovery_t[train_index]
#testing data
test_dat=dat[-train_index,]
x2=model.matrix(recovery_t~., data=dat)[-train_index,-1]
y2=dat$recovery_t[-train_index]  
```
...
```{r}
par(mfrow=c(2,2)) 
boxplot(recovery_t~study, data=dat, xlab="study", ylim=c(0,150))
boxplot(recovery_t~gender, data=dat, xlab="gender", ylim=c(0,150))
boxplot(recovery_t~hypertension, data=dat, xlab="hypertension", ylim=c(0,150))
boxplot(recovery_t~diabetes, data=dat, xlab="diabetes", ylim=c(0,150))
boxplot(recovery_t~vaccine, data=dat, xlab="vaccine", ylim=c(0,150))
boxplot(recovery_t~severity, data=dat, xlab="severity", ylim=c(0,150))
boxplot(recovery_t~race, data=dat, xlab="race", ylim=c(0,150))
boxplot(recovery_t~smoking, data=dat, xlab="smoking", ylim=c(0,150))
...
#tree
set.seed(2023)
rpart_fit <- train(recovery_t ~., data = dat[train_index,], 
                     method = "rpart",
