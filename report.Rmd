...
Among all the eleven models, according to Figure 3, Boosting, MARS and Bagging (Random Forest) have smaller RMSE than other models. 

```{r,echo=FALSE,out.width='90%',out.height='70%',fig.align='center'}
knitr::include_graphics("primary_analysis_plot/resample_plot.png")
```
\begin{center}
Figure 3. Distribution of RMSE of the eleven models 
\end{center}

On the other hand, we predicted the outcome with the test data by the eleven models. According to Table 1, we found that Boosting, MARS and GAM have the greatest performance of predicting the values of outcome.
...
Therefore, we selected Boosting as the best model to predict time to recovery.

## Secondary Analysis

For classification, we considered time to recovery as a binary outcome (>30 days vs. <= 30 days) and aim to develop a prediction model for this binary outcome. According to Figure 4, the Boosting model has the smallest RMSE, so we selected it as the best model to predict the binary value of time to recovery.
