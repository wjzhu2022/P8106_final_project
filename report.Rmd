...
In supervised learning, a linear model is a straightforward technique that supposes the outcome relies on the predictors linearly and that the errors follow a normal distribution with uniform variance. The train() function with 10-fold cross-validation was used to fit this linear model 5 times, as specified by the trainControl() function, using linear regression. Statistical information about the model was obtained by summarizing it. Additionally, the root-mean-square error (RMSE) was computed by comparing predicted and actual recovery time values.
# Primary Analysis
