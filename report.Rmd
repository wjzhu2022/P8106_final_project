...
In supervised learning, a linear model assumes $Y=\beta_0+\beta_1X+\epsilon$, in which the $\epsilon$ follows a normal distribution with uniform variance. The train() function with 10-fold cross-validation was used to fit this linear model 5 times, as specified by the trainControl() function, using linear regression. Statistical information about the model was obtained by summarizing it. Additionally, the RMSE was computed by comparing predicted and actual recovery time values.
# Primary Analysis
