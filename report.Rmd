...
As for classification, shown in Figure 5, the boosted tree model has the smallest mean and median RMSE, so we selected it as the best model, same as regression, to predict the binary value of time to recovery from COVID-19. And to estimate the performance of the final model, we plotted the ROC curve, shown in Figure 6. The prediction Accuracy for classification is xxxx%, which means that the correct predictions will consist of xxxx% of all predictions on average. And the most important variable in the final model for classification is …….
...
To identify important risk factors for longer recovery time and gain a better understanding of the predictors of recovery time from COVID-19, we developed prediction models with a variety of techniques. Among them, the boosted tree model shows the greatest performance on predicting time to recovery from COVID-19. 